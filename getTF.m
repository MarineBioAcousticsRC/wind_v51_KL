...
if tfd <= 300
    ...
    TFsFold = [p.tf.TFsFolder,'\',num2str(tfd),'-',num2str(tfd99),'\*'];
    ...
    if exist(TFsFold)
        ...
        while isempty(tfMatch) && iTF<=size(tfList,1)
            tfMatch = strfind(tfList(iTF).name,p.tf.tfn);
            iTF = iTF+1;
        end
        if ~isempty(tfMatch)
            tfMatchIdx = iTF-1;
...
if ~exist('suggestedTFPath','var')
    suggestedTFPath = p.tf.TFsFolder;
end
if exist('tfNum','var')
    disp(['TF number: ',num2str(tfNum)]);
end
[tf_file, tf_pathname,~ ] = ...
    uigetfile('*.tf','Pick Transfer Function',suggestedTFPath);
