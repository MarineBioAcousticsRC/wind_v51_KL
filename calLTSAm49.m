...
for iyr = 1 :  length(wyrs)
    ...
    elseif exist([WindFolder,num2str(wyrs(iyr)),'\',Proj,Site,'windvec.mat'])
        load([WindFolder,num2str(wyrs(iyr)),'\',Proj,Site,'windvec.mat'],'dnvec','wspeed');
    elseif exist([WindFolder,num2str(wyrs(iyr)),'\',Site,'windvec.mat'])
        load([WindFolder,num2str(wyrs(iyr)),'\',Site,'windvec.mat'],'dnvec','wspeed');
    elseif exist([WindFolder,num2str(wyrs(iyr)),'\',Proj,Short,'windvec.mat'])
        load([WindFolder,num2str(wyrs(iyr)),'\',Proj,Short,'windvec.mat'],'dnvec','wspeed');
    elseif exist([WindFolder,num2str(wyrs(iyr)),'\',Site,Short,'windvec.mat'])
        load([WindFolder,num2str(wyrs(iyr)),'\',Site,Short,'windvec.mat'],'dnvec','wspeed');
    elseif exist(p.windfile{iyr,1},'file')
        load(p.windfile{iyr,1},'dnvec','wspeed');
    else
        disp('Could Not Find Wind File');
        suggestedWind = fullfile(WindFolder,num2str(wyrs(iyr)));
        [w_file, w_pathname ] =...
            uigetfile(fullfile(suggestedWind,'*.mat'),'Pick Wind File');
        load([w_pathname,w_file],'dnvec','wspeed');
        p.windfile{iyr,1} = fullfile(w_pathname,w_file);
    end
