...
% JAH 10/2019  10/2020 update for lastest A = TF and Knudsen6

clear variables
%
RegPlt = 'on'; % show regression plots off or on
FrePlt = 'on'; % show freq plots for each sea state off or on
WndPlt = 'off'; % show wind speed plot  off or on
UpdateTF = 'off'; % use iteration of TF on or off
global PARAMS
...
%
% Batch process WindNoise files
% get filenames
[fwn_files, fwn_pathname] = uigetfile( ...
    '*.mat','Pick WindNoise(s)',...
    'MultiSelect','on');
% sort if multiple files selecte
fwn_files = sort(fwn_files);
% from  name, determine deployment and depth
for indexf = 1 : length(fwn_files)
    dBaseName = strrep(fwn_files{indexf},'_WindNoise.mat','');
    OutVsFreq = [dBaseName,'_VsFreq','.fig'];
    OutVsWind = [dBaseName,'_VsWind','.fig'];
    OutTFCorr = [dBaseName,'_TFCorr','.mat'];
    
    % load WindNoise.mat file either newly created or previously stored
    ffile = cell2mat(fullfile(fwn_pathname,fwn_files(indexf)));
    if exist(ffile)
        load(ffile);
    else
    ...
    if strcmp(UpdateTF,'on') %
        ...
        Ptf1 = interp1(tf_freq,tf_uppc,freq,'linear','extrap');
        Ptf =  repmat(Ptf1',1,length(mpwr));
        mpwrtf = mpwr + Ptf;   % add tran
    end
    %     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Hack to catch no depth
    if (~exist('depth'))
        depth = 1000;
    elseif isnan(depth)
        depth=1000;
    ...
    if tfn > 600  && fs0 > 100000 % skip for old TF or decimated data
        ...
        for i = 1:length(eltsam)-1  % used to be fnum
    ...
    itf = 1;
    TFCorr = zeros(8,1000); % ss 2 to ss =8, 100 Hz increments to 100kHz
    ...
    for  i = 2 : 9    % start at i = 2 ss1 end i=9 ss8
        if ~isempty(MPTF{i})
            smptf = size((MPTF{i}(2:1001,:)')); % 100 Hz to 100 kHz
            if smptf(1) > 1
                AM = mean(MPTF{i}(2:1001,:)');
            elseif smptf(1) == 1
                AM = MPTF{i}(2:1001,:)';
            end
            TFCorr(itf,:) = kd(i,1:1000) - AM; % first value is 100 Hz
            for j = 1 : length(ifr)
            ...
            if strcmp(FrePlt,'on')
                ...
                hold on
                semilogx(freq(2:301),AM(1:300),'r','LineWidth',3); % from 100 Hz to 100 kHz
    ...
    %Noise Model vs Wind figure
    %if strcmp(RegPlt,'on')
    NMFig = figure( 'Name', 'Noise vs Log10Wind' );
...
%     'Blue','Brown','Cyan','Green','Magenta','Orange','Purple','Red'};
    for ims = 1 : length(ifr)-1
        plot(log10(ms(2:9)),kd(2:9,ifr(ims)*10),'color',rgb(c{ims}))
        hold on
        plot(log10(ms(2:9)), (kd(2:9,10*ifr(ims))-TFCorrWS(:,ims)),...
            'o','color',rgb(c{ims}))
        ax = gca;
    ...
    title(ttitle);
    %end
    isel = [1,5,10,12,13,15,17,18]; % pick freq for plotting and slope
    ifselect = ifr(isel);
    % REgress
    [SlopeLR,OffSetLR,SlopeTS,OffSetTS,fTS] = WNRegress(...
        OutFolder,OutVsWind,Proj,Site,Depl,ttitle,depth,...
        wsnew,mpwrtf,ptime,dnew,kd,ms,fs0,ifselect,RegPlt);
    %
    % Make TF Correction
    ...
    save(tfcorrfile,'fre','MTFCorr','TFCorrWS','depth','tfn','SlopeTS','OffSetTS','fTS');
    % Make New TF
