function [ptimel,mpwrl,mpwrtfl,freql,eltsal,dfreql,dnew,wsnew,fs0l] = calLTSAl49(fn_files, fn_pathname)
...
OutNamel = p.harp.OutNamel;
NA = p.harp.NA ;     % number of time slices (spectral averages) to read per raw file
tres = p.harp.tres ;
% %
% [fn_files, fn_pathname,~] = uigetfile('*.ltsa','Pick Low-LTSA(s)',LTSAFolderl,...
%     'MultiSelect','on');
...
%REMOVE TIMES outside of 2006 - 2023
...
for i = 1 :  length(wyrs)
    ...
    elseif exist([WindFolder,num2str(wyrs(i)),'\',Proj,Site,'windvec.mat'])
        load([WindFolder,num2str(wyrs(i)),'\',Proj,Site,'windvec.mat'],'dnvec','wspeed');
    elseif exist([WindFolder,num2str(wyrs(i)),'\',Site,'windvec.mat'])
        load([WindFolder,num2str(wyrs(i)),'\',Site,'windvec.mat'],'dnvec','wspeed');
    elseif exist([WindFolder,num2str(wyrs(i)),'\',Proj,Short,'windvec.mat'])
        load([WindFolder,num2str(wyrs(i)),'\',Proj,Short,'windvec.mat'],'dnvec','wspeed');
    elseif exist([WindFolder,num2str(wyrs(i)),'\',Site,Short,'windvec.mat'])
        load([WindFolder,num2str(wyrs(i)),'\',Site,Short,'windvec.mat'],'dnvec','wspeed');
    elseif exist(p.windfile{i,1},'file')
        load(p.windfile{i,1},'dnvec','wspeed');
    else
        disp('Could Not Find Wind File');
        suggestedWind = fullfile(WindFolder);
        disp(['Year: ',num2str(wyrs(i))])
        [w_file, w_pathname ] =...
            uigetfile(fullfile(suggestedWind),'Pick Wind File');
        load([w_pathname,w_file],'dnvec','wspeed');
        p.windfile{i,1} = fullfile(w_pathname,w_file);
    end
...
save(fullfile(OutFolder,'WindNoiseMat',OutNamel));%,...
