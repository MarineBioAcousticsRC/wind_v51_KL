...
% 141103 smw
clear variables
calavg = 'yes'; % calculate LTSA Average ? yes or no othewise read in previous
...
revise = 'yes'; % allow revision of TF cutoff frequencies
global PARAMS
PARAMS.harp.Proj = 'SOCAL';
PARAMS.harp.Site = '_CINMSB_';
PARAMS.harp.Depl = '09';
...
if fs0 == 200000 ||  fs0 == 320000 || fs0 == 64000 || ...
    ...
    if tfd <= 300
        ...
        isokb = find( mpwrtf(iften,:) > 30 & mpwrtf(iften,:) < 60); %10 kHz
    end
    isok = intersect(isoka,isokb);
elseif fs0 == 10000
    ifone = find(freq < 100 + dfreq/2 & freq > 100 - dfreq/2); % 100 Hz
    iffive = find(freq < 500 + dfreq/2 & freq > 500 - dfreq/2); % 500 Hz
    isok = find( mpwrtf(ifone,:) > 30 & mpwrtf(ifone,:) < 90); %100 Hz = 10 * 10
else
...
if fs0 == 200000 || fs0 == 320000 || fs0 == 64000 || ...
        fs0 == 96000 || fs0 == 48000
    plot(wsnew(iwind),mpwrtf(ifone,inoise),'o') %use 1 kHz noise
    hold on
    plot(wsnew(iwind),mpwrtf(iften,inoise),'ro') %use 10 kH
    legend('1 kHz','10 kHz','Location','southeast');
elseif fs0 == 10000
    plot(wsnew(iwind),mpwrtf(ifone,inoise),'o') %use 100 Hz noise
    hold on
    plot(wsnew(iwind),mpwrtf(iffive,inoise),'ro') %use 500 Hz
...
[SlopeLR,OffSetLR,SlopeTS,OffSetTS,fTS] = WNRegress(...
    ...
    wsnew,mpwrtf,ptime,dnew,kd,ms,fs0,ifr,freq,dfreq,RegPlt);
%
% Make TF Correction
