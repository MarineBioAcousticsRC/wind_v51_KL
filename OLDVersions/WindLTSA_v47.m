% Compare Wind to LSTA - SINGLE DEPLOYMENT uses windTF from pub
% v45 - add mid band  11/2020 - v47 no need to move midband 1/2021
% JAH 10/2019
% derived from LTSAdailySpectra.m
% 141103 smw
clear variables
calavg = 'y'; % calculate LTSA Average ? y or n othewise read in previous
calavgm = 'y'; % calculate LTSAm Average ? y or n othewise read in previous
RegPlt = 'on'; % show regression plots off or on
FrePlt = 'on'; % show freq plots for each sea state off or on
NMPlt = 'on'; % show Noise Model plot for each sea state off or on
...
PARAMS.ltsa.LTSAFolder = ['G:\LTSA\',PARAMS.harp.Proj,'\',PARAMS.harp.Proj,...
    '_',PARAMS.harp.Site,'_',PARAMS.harp.Depl];
PARAMS.tf.TFsFolder = 'P:\Shared drives\MBARC_TF';
% PARAMS.tf.TFsFolder = 'H:\Wind_TF\Output\Kona_TF\TF_Wind'; %uses wind TF
...
if strcmp(calavgm,'y') % calculate WindNoise.mat file ?
    [ptimem,mpwrm,mpwrtfm,freqm,eltsam] = calLTSAm;  % reads LTSAm files to get averages and gets Wind model
    load(fullfile(PARAMS.harp.OutFolder,'WindNoiseMat',PARAMS.harp.OutName1)); % load full to get all parms
else
