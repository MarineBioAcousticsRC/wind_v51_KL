...
Wfig = figure(55); % Wind versus Noise plot
plot(wsnew(iwind),mpwrtf(11,inoise),'o') %use 1 kHz noise
hold on
plot(wsnew(iwind),mpwrtf(101,inoise),'ro') %use 1 kH
...
TFnew(miP+1:miP+1000,:) = [freq(2:end)',Ptf(2:end)']; % begin at 100 Hz
TFold = TFnew;
...
grid on
% save in inverse sensitivity tf format
