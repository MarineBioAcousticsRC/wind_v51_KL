...
% v100 modified for Arctic data including ice
...
global p
drivelet = 'G'; Proj = 'CANARC'; Site = 'LI'; Depl = '01';
...
[p.tf.freq,p.tf.uppc,p.tf.tffile] = ...
    getTFx(p.tf.TFsFolder); % Save tf incase it gets overwritten in recall
if strcmp(p.usek,'y') % add Kait TF
    [kaitFr,kaitTF] = getKTF(p.tf.KTFsFolder);
end
Psave = p;
% get LTSA locations
%
if strcmp(Psave.usel,'y') && strcmp(p.calavgl,'y')
    ...
    if exist(ltsapath)
        [fn_filesl, fn_pathnamel,~] = uigetfile('*.ltsa','Pick Low-LTSA(s)',...
...
if strcmp(Psave.usem,'y') && strcmp(p.calavgm,'y')
    ...
    if exist(ltsapath)
        [fn_filesm, fn_pathnamem,~] = uigetfile('*.ltsa','Pick Mid-LTSA(s)',...
            ltsapath, 'MultiSelect','on');
    else
        [fn_filesm, fn_pathnamem,~] = uigetfile('*.ltsa','Pick Mid-LTSA(s)',...
            ltsabase, 'MultiSelect','on');
...
if strcmp(Psave.use,'y') && strcmp(p.calavg,'y')
    ...
    if exist(ltsapath)
        [fn_files, fn_pathname,~] = uigetfile('*.ltsa','Pick LTSA(s)',...
            ltsapath, 'MultiSelect','on');
    else
        [fn_files, fn_pathname,~] = uigetfile('*.ltsa','Pick LTSA(s)',...
            ltsabase, 'MultiSelect','on');
...
% get ice data file
% Define the patterns
patterns = {Proj,Site,Depl};

% Get a list of all files in the directory
icefiles = dir(p.icepath);
% Initialize a variable to store the found file name
foundFile = '';
% Loop through each file in the directory
for i = 1:length(icefiles)
    % Get the file name
    fileName = icefiles(i).name;

    % Check if the file name contains all the patterns
    if contains(fileName, patterns) && contains(fileName, '.csv')
        % Store the found file name
        foundFile = fileName;
        break;
    end
end
...
if ~isempty(foundFile)
    ...
    fullPath = fullfile(p.icepath, foundFile);
    % Load the CSV file
    icedata = readtable(fullPath);
    % Display a message with the found file name
    fprintf('Ice file: %s\n', foundFile);
else
    % Display a message if no file was found
    fprintf('No Ice file matching the patterns was found.\n');
end
...
[ptimel,mpwrl,mpwrtfl,freql,eltsal,dfreql,dnew,wsnew,fs0l] = ...
        calLTSAl51(fn_filesl, fn_pathnamel);  % reads LTSA files to get averages and gets Wind model
    else
...
if strcmp(Psave.usem,'y')
    if strcmp(Psave.calavgm,'y') % calculate WindNoise_mid.mat file ?
        [ptimem,mpwrm,mpwrtfm,freqm,eltsam,dfreqm,dnew,wsnew,fs0m] = ...
            calLTSAm51(fn_filesm, fn_pathnamem);  % reads LTSAm files to get averages and gets Wind model
    else
...
if strcmp(Psave.use,'y')
    if strcmp(Psave.calavg,'y') % calculate WindNoise.mat file ?
        [ptime,mpwr,mpwrtf,freq,eltsa,dfreq,dnew,wsnew,fs0] = ...
            calLTSA51(fn_files, fn_pathname);  % reads LTSA files to get averages and gets Wind model
...
while strcmp(revise,'d')
    ...
    if strcmp(p.usem,'y') %figure 40
        ...
        figure(40);
        savefig(wnfignam40)
        [MPTFm,lform] = WindSort49(wsfinalm,mpwfinalm); % sort into speed
        lfor = min([lfor, lform]);
        %TF Correction
        TFCorrm = NaN(8,nfm-1); %
    end
    if strcmp(p.usel,'y') %figure 400
        ...
        figure(400);
        savefig(wnfignam400)
        [MPTFl,lforl] = WindSort49(wsfinall,mpwfinall); % sort into speed
        lfor = min([lfor,lforl]);
        %TF Corr low
