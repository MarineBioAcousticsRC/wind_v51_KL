function [ptime,mpwr,mpwrtf,freq,eltsa,dfreq,dnew,wsnew,fs0] = calLTSA49(fn_files, fn_pathname)
global p PARAMS
Proj = p.harp.Proj ;
Site = p.harp.Site ;
Short = p.harp.Short ;
...
for m = 1:nm    % loop over time average bins
    ...
    for f = 1:nfiles    % loop over files with same month (probably only 2 max)
        ...
        if fs0 == 2000 || fs0 == 200000 || fs0 == 10000 || fs0 == 64000 || fs0 == 50000 || fs0 == 80000
            %  IA = (15+5)*nf;
            IA = (nave + NA)*nf;  % better than commented 200kHz hardwired above
        elseif fs0 == 3200 || fs0 == 320000 || fs0 == 32000
            IA = (nave + NA-1)*nf;
        else
...
for i = 1 :  length(wyrs)
    ...
    elseif exist(p.windfile{i,1},'file')
        ...
        suggestedWind = fullfile(WindFolder,num2str(wyrs(i)));
        [w_file, w_pathname ] =...
