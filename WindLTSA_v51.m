...
global p
drivelet = 'Z'; Proj = 'GofMX'; Site = 'Y5A'; Depl = '01';
p = getWindParams(drivelet,Proj,Site,Depl); % paramter file
% Start capturing the command window. Everything from here on - the values
% typed at the prompts, the files chosen in each dialog, warnings and any
% error - lands in a timestamped log under p.harp.OutLog. Closed by the
% 'diary off' at the very end of this script.
runLogFile = startRunLog(p);
% Get TF - allows update without recalculating LTSA averages
tfNum = input('Enter TF number: ');
p.tf.tfn = num2str(tfNum);
prompt = ' Enter Depth in m: ';
p.tf.depth = input(prompt);
% tell the dialog which hydrophone and deployment this is for
tfNote = sprintf('TF %s  -  %s %s depl %s  -  %g m', ...
    p.tf.tfn,Proj,Site,Depl,p.tf.depth);
[p.tf.freq,p.tf.uppc,p.tf.tffile] = ...
    getTFx(p.tf.TFsFolder,tfNote); % Save tf incase it gets overwritten in recall
if strcmp(p.usek,'y') % add Kait TF
    [kaitFr,kaitTF] = getKTF(p.tf.KTFsFolder,['Kait TF  -  ',tfNote]);
end
Psave = p;
% get LTSA locations
%
if strcmp(Psave.usel,'y') && strcmp(p.calavgl,'y')
    ltsabase = p.ltsa.LTSAFolder;
    ltsapath = fullfile(ltsabase,Site,'df100');
    ltsaNotel = sprintf('Pick LOW-band LTSA(s)  [df100]  -  %s %s depl %s',Proj,Site,Depl);
    if exist(ltsapath)
        [fn_filesl, fn_pathnamel,~] = uigetfile('*.ltsa',ltsaNotel,...
            ltsapath, 'MultiSelect','on');
    else
        [fn_filesl, fn_pathnamel,~] = uigetfile('*.ltsa',ltsaNotel,...
            ltsabase, 'MultiSelect','on');
        p.ltsa.LTSAFolder = fn_pathnamel;
    end
end
if strcmp(Psave.usem,'y') && strcmp(p.calavgm,'y')
    ltsabase = p.ltsa.LTSAFolder;
    ltsapath = fullfile(ltsabase,Site,'df20');
    ltsaNotem = sprintf('Pick MID-band LTSA(s)  [df20]  -  %s %s depl %s',Proj,Site,Depl);
    if exist(ltsapath)
        [fn_filesm, fn_pathnamem,~] = uigetfile('*.ltsa',ltsaNotem,...
            ltsapath, 'MultiSelect','on');
    else
        [fn_filesm, fn_pathnamem,~] = uigetfile('*.ltsa',ltsaNotem,...
            ltsabase, 'MultiSelect','on');
        p.ltsa.LTSAFolder = fn_pathnamem;
    end
end
if strcmp(Psave.use,'y') && strcmp(p.calavg,'y')
    ltsabase = p.ltsa.LTSAFolder;
    ltsapath = fullfile(ltsabase,Site,'df1');
    ltsaNoteh = sprintf('Pick HIGH-band LTSA(s)  [df1]  -  %s %s depl %s',Proj,Site,Depl);
    if exist(ltsapath)
        [fn_files, fn_pathname,~] = uigetfile('*.ltsa',ltsaNoteh,...
            ltsapath, 'MultiSelect','on');
    else
        [fn_files, fn_pathname,~] = uigetfile('*.ltsa',ltsaNoteh,...
            ltsabase, 'MultiSelect','on');
        p.ltsa.LTSAFolder = fn_pathname;
    end
end
...
if strcmp(Psave.usel,'y')
    if strcmp(Psave.calavgl,'y') % calculate WindNoise_low.mat file ?
        [ptimel,mpwrl,mpwrtfl,freql,eltsal,dfreql,dnew,wsnew,fs0l] = calLTSAl51(fn_filesl, fn_pathnamel);  % reads LTSA files to get averages and gets Wind model
    else
        % if low exists load it
        if exist(fullfile(p.harp.OutFolder,'WindNoiseMat',p.harp.OutNamel))
            disp(['load: ',p.harp.OutNamel])
            load(fullfile(p.harp.OutFolder,'WindNoiseMat',p.harp.OutNamel),...
                'ptimel','mpwrl','mpwrtfl','freql','eltsal','dfreql','dnew','wsnew','p','fs0l');
...
if strcmp(Psave.usem,'y')
    if strcmp(Psave.calavgm,'y') % calculate WindNoise_mid.mat file ?
        [ptimem,mpwrm,mpwrtfm,freqm,eltsam,dfreqm,dnew,wsnew,fs0m] = calLTSAm51(fn_filesm, fn_pathnamem);  % reads LTSAm files to get averages and gets Wind model
    else
        % if mid exists load it
        if exist(fullfile(p.harp.OutFolder,'WindNoiseMat',p.harp.OutNamem))
            disp(['load: ',p.harp.OutNamem])
            load(fullfile(p.harp.OutFolder,'WindNoiseMat',p.harp.OutNamem),...
                'ptimem','mpwrm','mpwrtfm','freqm','eltsam','dfreqm','dnew','wsnew','p','fs0m');
            if p.tf.uppc ~= Psave.tf.uppc
...
if strcmp(Psave.use,'y')
    if strcmp(Psave.calavg,'y') % calculate WindNoise.mat file ?
        [ptime,mpwr,mpwrtf,freq,eltsa,dfreq,dnew,wsnew,fs0] = calLTSA51(fn_files, fn_pathname);  % reads LTSA files to get averages and gets Wind model
        load(fullfile(p.harp.OutFolder,'WindNoiseMat',p.harp.OutName));
    else
        if exist(fullfile(p.harp.OutFolder,'WindNoiseMat',p.harp.OutName))
            disp(['load: ',p.harp.OutName])
            load(fullfile(p.harp.OutFolder,'WindNoiseMat',p.harp.OutName),...
                'ptime','mpwr','mpwrtf','freq','eltsa','dfreq','dnew','wsnew','p','fs0');
            if p.tf.uppc ~= Psave.tf.uppc
...
Psave.ltsa =  p.ltsa;
% carry forward things the cal/load steps discovered, otherwise restoring
% Psave throws away the wind provenance and any file the user just picked
if isfield(p,'windInfo')
    Psave.windInfo = p.windInfo;
end
if isfield(p,'windSource')
    Psave.windSource = p.windSource;
end
if isfield(p,'windfile')
    Psave.windfile = p.windfile;
end
p = Psave;
depth = p.tf.depth;
tf_file = p.tf.tffile(end-19:end);
...
if strcmp(Psave.usem,'y') || strcmp(Psave.usel,'y')
    if strcmp(Psave.usem,'y')
        fm = .01 : .01 : (freqm(end))/1000; % 10 Hz - 5 khz in 10 Hz steps
        kdm = zeros(11,freqm(end)/10);
    else
        fm = .01 : .01 : 1; % 10 Hz - 1 khz in 10 Hz steps
        kdm = zeros(11,100);
    end

    for i = 1:11
        kdm(i,:) = interp1([fnm(1:10),fnm(12:90)],...
            [kdp(i,1:10),kdp(i,12:90)],fm); %10 Hz -5 kHz 0r 8 kHz for 320 phone
    end
end
kd = kdp(:,11:end);
% plot Knudsen curves
...
if strcmp(Psave.usem,'y')
    ...
    if300m = find(freqm < 300 + dfreqm/2 & freqm > 300 - dfreqm/2); % 300 Hz
...
%% Check wind/noise time matching BEFORE fitting
% createFit4/createFit5 fit a poly5, which needs at least 6 points. When the
% wind file does not line up with the LTSA times, the old code fell over
% deep inside fit() with a bare "Insufficient data" message. Fail here
% instead, with the numbers needed to work out why.
if ~isfield(p,'minFitPts') || isempty(p.minFitPts)
    p.minFitPts = 20;
end
if ~isfield(p,'windSource') || isempty(p.windSource)
    p.windSource = 'unlabelled';
end
fprintf('\n--- Wind / noise time matching ----------------------------\n');
fprintf('Wind source : %s\n',p.windSource);
if isfield(p,'windInfo') && isfield(p.windInfo,'files')
    for iwf = 1 : numel(p.windInfo.files)
        fprintf('Wind file   : %s\n',p.windInfo.files{iwf});
    end
end
fprintf('Wind series : %d samples, %s to %s\n',numel(dnew), ...
    datestr(dnew(1),'yyyy-mm-dd'),datestr(dnew(end),'yyyy-mm-dd'));

bandName = {}; bandN = []; bandT0 = []; bandT1 = [];
if strcmp(p.usel,'y')
    bandName{end+1} = 'Low';  bandN(end+1) = numel(iXl);
    bandT0(end+1) = ptimel(1); bandT1(end+1) = ptimel(end);
end
if strcmp(p.usem,'y')
    bandName{end+1} = 'Mid';  bandN(end+1) = numel(iXm);
    bandT0(end+1) = ptimem(1); bandT1(end+1) = ptimem(end);
end
if strcmp(p.use,'y')
    bandName{end+1} = 'High'; bandN(end+1) = numel(iX);
    bandT0(end+1) = ptime(1);  bandT1(end+1) = ptime(end);
end
for ib = 1 : numel(bandName)
    fprintf('%-4s band  : %6d matched wind/noise points   (LTSA %s to %s)\n', ...
        bandName{ib},bandN(ib), ...
        datestr(bandT0(ib),'yyyy-mm-dd'),datestr(bandT1(ib),'yyyy-mm-dd'));
end
fprintf('-----------------------------------------------------------\n\n');

% Two thresholds, deliberately. 6 is the hard floor - poly5 in createFit4
% and createFit5 cannot fit fewer than that, so the run has to stop.
% p.minFitPts is a quality floor: between 6 and it the fit is possible but
% badly over-fitted, so warn and carry on rather than killing an otherwise
% good run. One short band must NOT abort the other bands.
hardMin = 6;
iwarn = find(bandN >= hardMin & bandN < p.minFitPts);
for ib = iwarn
    fprintf(2,['WARNING: %s band has only %d matched points, below ', ...
        'p.minFitPts = %d.\n         Its poly5 fit will be heavily ', ...
        'over-fitted - treat that band with suspicion.\n'], ...
        bandName{ib},bandN(ib),p.minFitPts);
end

ibad = find(bandN < hardMin);
if ~isempty(ibad)
    msg = sprintf(['Not enough matched wind/noise points to fit.\n', ...
        'createFit4/5 use poly5, which needs at least %d points.\n'],hardMin);
    for ib = ibad
        msg = [msg,sprintf('  %s band: only %d point(s)\n', ...
            bandName{ib},bandN(ib))]; %#ok<AGROW>
    end
    msg = [msg,sprintf([ ...
        '\nThe wind times and the LTSA times are not lining up. Check:\n', ...
        '  1. p.usebuoy / p.windfile / p.windFolderOverride in getWindParams.m\n', ...
        '     (wind source = "%s")\n', ...
        '  2. that the wind file period overlaps the LTSA period above\n', ...
        '  3. p.dEarly / p.dLate, which trim ptime inside calLTSA*51.m\n', ...
        '  4. that you picked LTSAs for the deployment you meant\n', ...
        '  5. windFileReport(p.harp.WindFolder) lists the wind you have\n'], ...
        p.windSource)];
    error('WindLTSA:tooFewPoints','%s',msg);
end
...
revise = 'd'; lfor = 9; % max wind used
while strcmp(revise,'d')
    if strcmp(p.use,'y') % figure 4
        wnfile = fullfile(p.harp.OutFolder,'WindNoiseMat',p.harp.OutName);
        ...
        [MPTF,lfor] = WindSort51(wsfinal,mpwfinal); % Mean Pressure TF corrected
        % Make TF Correction
        TFCorr = NaN(8,nf-1);
    end
    if strcmp(p.usem,'y') %figure 40
        wnfilem = fullfile(p.harp.OutFolder,'WindNoiseMat',p.harp.OutNamem);
        ...
        nfm = smpwm(1);
        figure(40); clf;
        plot(wsfinalm',mpwfinalm(if100m,:),'ko'); %use 50 Hz noise
        hold on
        plot(wsfinalm',mpwfinalm(if200m,:),'bo'); %use 100Hz noise
        plot(wsfinalm',mpwfinalm(if300m,:),'ro'); %use 100Hz noise
        plot(wsfinalm',mpwfinalm(if500m,:),'go'); %use 100Hz noise
        legend('100 Hz','200 Hz','300 Hz','500 Hz','Location','southeast');
        title([p.harp.dBaseName,' Mid']);
        wnfignam40 = fullfile(p.harp.OutWinFig,[p.harp.dBaseName,'WindNoise_mid']);
        figure(40);
        savefig(wnfignam40)
        [MPTFm,lform] = WindSort51(wsfinalm,mpwfinalm); % sort into speed
        lfor = min([lfor, lform]);
        %TF Correction
        TFCorrm = NaN(8,nfm-1); %
    end
    if strcmp(p.usel,'y') %figure 400
        wnfilel = fullfile(p.harp.OutFolder,'WindNoiseMat',p.harp.OutNamel);
        ...
        wnfignam400 = fullfile(p.harp.OutWinFig,[p.harp.dBaseName,'WindNoise_low']);
        figure(400);
        savefig(wnfignam400)
        [MPTFl,lforl] = WindSort51(wsfinall,mpwfinall); % sort into speed
        lfor = min([lfor,lforl]);
        %TF Corr low
    ...
    %     lfor = min([lfor,lform,lforl,9]); %limit to ss8
    for  iss = 2 : lfor    % start at iss = 2 ss1 end iss=9 ss8
        % Average MPTF: AMl AMm and AM start at 1Hz 10 Hz and 100 Hz
        if strcmp(p.use,'y') %
            smptf = size((MPTF{1,iss}(2:nf,:)')); % 200 Hz to 30 kHz
            if smptf(1) > 1
                AM = mean(MPTF{1,iss}(2:nf,:)');
            elseif smptf(1) == 1
                AM = MPTF{1,iss}(2:nf,:)';
            end
            TFCorr(itf,1:nf-1) = kd(iss,1:nf-1) - AM(1:nf-1); % first value is 100 Hz last highest freq
        end
        if strcmp(p.usem,'y') %
            smptfm = size((MPTFm{1,iss}(2:nfm,:)')); %
            if smptfm(1) > 1
                AMm = mean(MPTFm{1,iss}(2:nfm,:)'); % 20 Hz to 5 kHz
            elseif smptfm(1) == 1
                AMm = MPTFm{1,iss}(2:nfm,:)'; %
            end
            TFCorrm(itf,1:nfm-1) = kdm(iss,1:nfm-1) - AMm(1:nfm-1); % first value is 10 Hz last 1000 Hz
        end
        if strcmp(p.usel,'y') %
            smptfl = size((MPTFl{1,iss}(2:nfl,:)')); %
            if smptfl(1) > 1
                AMl = mean(MPTFl{1,iss}(2:nfl,:),2)'; % 2 Hz to 1 kHz
            elseif smptfl(1) == 1
                AMl = MPTFl{1,iss}(2:nfl,:)'; %
            end
            % correct for 1 Hz sampling of AMl and 10 Hz sampling of kdm
            TFCorrl10(itf,:) = kdm(iss,1:nfl10-1) - AMl(10:10:nfl-1); % first is 10 Hz last 1000 Hz
        end
        itf = itf + 1;

        %% Freq Plot
        if strcmp(p.FrePlt,'on')
            if strcmp(p.usem,'y') && strcmp(p.use,'y') && iplotmh <= 8 % mid + high
                if iplotmh ==1
                ...
                semilogx(freq(5:nf),MPTF{1,iss}(5:nf,:)); % from 200 Hz to 30 kHz
                hold on
                semilogx(freqm(3:41),MPTFm{1,iss}(3:41,:)); % from 20 Hz to 200 Hz
                semilogx(1000*fnm,kdp(iss,:),'r','LineWidth',3); % theory as a line
                ...
                grid on
                ftxt =['Beaufort Force' ,num2str(ss(iss))];
                text(1000,94,ftxt)
            ...
            %
            if strcmp(p.usel,'y') && strcmp(p.use,'y') && iplotlh <= 8 % low + high
                % low freq plot
                if iplotlh ==1
                ...
                semilogx(freq(5:nf),MPTF{1,iss}(5:nf,:)); % from 400 Hz to 100 kHz
                hold on
                semilogx(freql(2:401),MPTFl{1,iss}(2:401,:)); % from 10 Hz to 400 Hz
                semilogx(1000*fnm,kdp(iss,:),'r','LineWidth',3); % theory as a line
            ...
            %
            if strcmp(p.usel,'y') && iplotl <= 8 % low only freq plot
                if iplotl ==1
                    ssFigl = figure(600); clf;
                end
                figure(ssFigl);
                subplot(4,2,iplotl)  % 8 subplots = 4 x 2
                semilogx(freql(2:nfl),MPTFl{1,iss}(2:nfl,:)); % from 10 Hz to nfl Hz
                hold on
                semilogx(1000*fnm,kdp(iss,:),'r','LineWidth',3); % theory as a line
            ...
            %
            if strcmp(p.usem,'y') && iplotm <=8 % mid only freq plot
                if iplotm ==1
                    ssFigm = figure(60); clf;
                end
                figure(ssFigm);
                subplot(4,2,iplotm)  % 8 subplots = 4 x 2
                semilogx(freqm(2:nfm),MPTFm{1,iss}(2:nfm,:)); % from 10 Hz to nfm Hz
                hold on
                semilogx(1000*fnm,kdp(iss,:),'r','LineWidth',3); % theory as a line
            ...
            if strcmp(p.use,'y') && iplot <= 8% mid only freq plot
                if iplot ==1
                    ssFig = figure(6); clf;
                end
                figure(ssFig);
                subplot(4,2,iplot)  % 8 subplots = 4 x 2
                semilogx(freq(3:nf),MPTF{1,iss}(3:nf,:)); % from 200 Hz to 30 kHz
                hold on
                semilogx(freq(2:nf),kd(iss,1:nf-1),'r','LineWidth',3); % theory as a line
    ...
    if strcmp(p.use,'y')%  REgress high
        % Regression frequencies in kHz. The per-rate lists are unchanged -
        % they stop short of Nyquist on purpose (anti-alias roll-off), so
        % do not "helpfully" extend them. The added else and the trim only
        % stop ifr being undefined for an unlisted rate, and stop a request
        % landing outside the band: WNRegress1 aborts the whole regression
        % if any requested frequency has no matching bin.
        if (fs0 == 200000 || fs0 == 320000)% Frequencies for plotting
            ifr = [.2, .5, 1, 2, 5, 10, 20, 30, 40, 50, 75, 100]; % freq in kHz
        elseif (fs0 == 64000 || fs0 == 80000 || fs0 == 96000)
            ifr = [.2, .5, 1, 2, 5, 10, 20, 30]; % freq in kHz
        elseif fs0 == 48000 || fs0 == 50000
            ifr = [.2, .5, 1, 2, 5, 10, 20]; % freq in kHz
        else
            ifr = [.2, .5, 1, 2, 5, 10, 20, 30, 40, 50, 75, 100];
            fprintf(2,['fs0 = %g is not one of the listed sample rates - ', ...
                'using the default regression frequencies.\n'],fs0);
        end
        ifr = ifr(ifr*1000 <= freq(end));
        [SlopeLR,OffSetLR,LRr2,SlopeTS,OffSetTS,SlopeTSu,OffSetTSu,fTS] = WNRegress1(...
            p.harp.OutFolder,p.harp.OutVsWind,p.harp.Proj,p.harp.Site,p.harp.Depl,ttitle,depth,...
            wsfinal,mpwfinal,ptime,dnew,kd,ms,fs0,ifr,freq,...
            dfreq,p.RegPlt,8);
    end
    if strcmp(p.usem,'y') % REgress mid
        % Frequencies for plotting, kHz. Was hard-coded to stop at 1 kHz
        % regardless of the LTSA, which threw away the top two octaves of
        % the mid band: a df20 LTSA off a 200 kHz phone has fs0m = 10 kHz
        % and so carries data to 5 kHz. Now trimmed to freqm(end) instead,
        % so a genuinely 2 kHz-sampled mid band still stops at 1 kHz.
        ifrmAll = [.02,.05,.1,.2,.3,.4,.5,.6,1,2,5];
        ifrm = ifrmAll(ifrmAll*1000 <= freqm(end));
        [SlopeLRm,OffSetLRm,LRr2m,SlopeTSm,OffSetTSm,SlopeTSum,OffSetTSum,fTSm] = WNRegress1(...
            p.harp.OutFolder,p.harp.OutVsWindm,p.harp.Proj,p.harp.Site,p.harp.Depl,ttitle,depth,...
            wsfinalm,mpwfinalm,ptimem,dnew,kdm,ms,fs0m,ifrm,freqm,...
            dfreqm,p.RegPlt,80);
    end
    if strcmp(p.usel,'y') % REgress low
        % Frequencies for plotting, kHz. 1 kHz is Nyquist for a df100 LTSA
        % (fs0l = 2 kHz), so the list is unchanged in the usual case - the
        % trim only guards a low band with different bandwidth.
        ifrlAll = [.005,.01,.02,.05,.1,.2,.3,.4,.5,.6,.8,1];
        ifrl = ifrlAll(ifrlAll*1000 <= freql(end));
        [SlopeLRl,OffSetLRl,LRr2l,SlopeTSl,OffSetTSl,SlopeTSul,OffSetTSul,fTSl] = WNRegress1(...
            p.harp.OutFolder,p.harp.OutVsWindl,p.harp.Proj,p.harp.Site,p.harp.Depl,ttitle,depth,...
    ...
    if strcmp(p.usel,'y')
        ...
        elseif imaxSS == 3
            MTFCorrl10 = TFCorrl10(3:imaxSS,:);%starts with freq = 10 Hz
        end
        figure(900); clf;
        ...
        axis(v)
        grid on
        title([p.harp.dBaseName,' Hydrophone ',p.tf.tfn])
        xlabel('Frequency [Hz]')
        ...
        % record this band's cutoffs in Hz. col/coh are bin indices whose
        % scale differs per band, and they get reused across bands later in
        % this script, so keep a per-band copy that cannot be overwritten.
        cutlowHzl  = col * 10*dfreql;
        cuthighHzl = coh * 10*dfreql;
        % Transfer function correction vector
        Ptf = interp1(p.tf.freq,p.tf.uppc,freql10,'linear','extrap');
        [TFoldl, TFnewl] = tfmakel51(10*dfreql,nfl10,col, coh, freql10, Ptf, MTFCorrl10);
    ...
    if strcmp(p.usem,'y')
        ...
        elseif imaxSS == 3
            MTFCorrm = TFCorrm(3:imaxSS,:);%starts with freq = 10 Hz
        end
        nMTm = isnan(MTFCorrm); % correct Nan
        inMTm = find(nMTm > 0); % indecies with Naans that need fixing 
        if (~isempty(inMTm)) % so where there are Naans, continue 
            for i = 1 : length(inMTm)-1
                MTFCorrm(inMTm(i))=(MTFCorrm(inMTm(i)-1)+MTFCorrm(inMTm(i)+1))/2 ; % hit error here on 888, "Index exceeds the number of array elements. Index must not exceed 1000."
            end
        end
        figure(90); clf;
        ...
        cutlowHzm  = col * dfreqm;   % per-band copy in Hz, see note above
        cuthighHzm = coh * dfreqm;
        %         freqmten = freqm(11:10:nfm);
        % Transfer function correction vector
        Ptf = interp1(p.tf.freq,p.tf.uppc,freqm,'linear','extrap');
        [TFoldm, TFnewm] = tfmakel51(dfreqm,nfm,col, coh, freqm, Ptf, MTFCorrm);
    ...
    if strcmp(p.use,'y') %
        % for high freq use only ss1 - ss7
         if imaxSS > 3
            MTFCorr = mean(TFCorr(3:imaxSS,:),'omitnan');%starts with freq = 10 Hz
        elseif imaxSS == 3
            MTFCorr = TFCorr(3:imaxSS,:);%starts with freq = 10 Hz
        end
        ...
        hold on
        if strcmp(p.usem,'y')
            MTFCorra = [MTFCorrm(10:10:40),MTFCorr(5:end)]; %replace points 100-500 Hz
        ...
        if strcmp(p.usel,'y')
            semilogx(freql10(2:end),MTFCorrl10,'r-.','LineWidth',3); %
            if ~strcmp(p.usem,'y')
                MTFCorra = [MTFCorrl10(11),MTFCorra(2:end)]; %replace point at 100 Hz
                semilogx(freq(2:nf),MTFCorra(1:nf-1),'k:','LineWidth',3); %
            end
        end
        v = [10 (nf-1)*dfreq -10 max(5,max(MTFCorra)+0.5)];
        axis(v)
        grid on
        title([p.harp.dBaseName,' Hydrophone ',p.tf.tfn])
        xlabel('Frequency [Hz]')
        ...
        cutlowHz  = col * dfreq;     % per-band copy in Hz, see note above
        cuthighHz = coh * dfreq;
        % Transfer function correction vector
        Ptf = interp1(p.tf.freq,p.tf.uppc,freq,'linear','extrap');
        [TFold, TFnew] = tfmake51(dfreq,nf,col, coh, freq, Ptf, MTFCorra,p.tf.tfn);
        %Make TF Figure
        TFFig = figure(10); clf;
        [TFFig] = tffigmake(TFFig,TFnew,TFold,tf_file,p.harp.dBaseName,col,coh);
        % code to compare to KaitFT
        if strcmp(p.usek,'y')
            kaitF = kaitFr(1):100:kaitFr(end)+500;
            kaitT = interp1(kaitFr,kaitTF,kaitF,'linear','extrap');
            TFwind = TFnew;
            [TFnewK] = tfmakeK(TFwind,coh,dfreq,kaitF,kaitT);
            KTFFig = figure(11); clf;
            [KTFFig] = tffigmakeKAIT(KTFFig,TFnewK,TFwind,TFold,tf_file,...
                p.harp.dBaseName,kaitF,kaitT);
        end
    end

    %% Revise or Save data
    revise = [];
    while ~(isequal(revise,'d') || isequal(revise,'c') || ...
            isequal(revise,'x') || isequal(revise,'q'))
    ...
    elseif strcmp(revise,'c')
        while (revise == 'c')
            ...
            if strcmp(p.use,'y') %high freq
                ...
                cutlowHz  = col * dfreq;    % what was actually used, in Hz
                cuthighHz = coh * dfreq;
                Ptf = interp1(p.tf.freq,p.tf.uppc,freq,'linear','extrap');
                [TFold, TFnew] = tfmake51(dfreq,nf,col, coh, freq, Ptf, MTFCorra,p.tf.tfn);
                figure(TFFig); clf;
            ...
            if  strcmp(p.usem,'y') % mid freq
                ...
                cutlowHzm  = col * dfreqm;  % post-clamp, in Hz
                cuthighHzm = coh * dfreqm;
                Ptf = interp1(p.tf.freq,p.tf.uppc,freqm,'linear','extrap');
                [TFoldm, TFnewm] = tfmakel51(dfreqm,nfm,col, coh, freqm, Ptf, MTFCorrm);
                figure(TFFigm); clf;
            ...
            if  strcmp(p.usel,'y') % low freq
                ...
                cutlowHzl  = col * 10*dfreql;   % post-clamp, in Hz
                cuthighHzl = coh * 10*dfreql;
                Ptf = interp1(p.tf.freq,p.tf.uppc,freql10,'linear','extrap');
                [TFold, TFnewl] = tfmakel51(10*dfreql,nfl10,col, coh, freql10, Ptf, MTFCorrl10);
                figure(TFFigl); clf;
    ...
    elseif strcmp(revise,'x') % bad result move files to "bad" folder
        p.SaveTF = 'no';
        if strcmp(p.use,'y') %
            [stat1,stat2,stat3,stat4] = movebad(wnfile,ssfile,wnfignam4);
        end
        if strcmp(p.usem,'y') %
            [stat1,stat2,stat3,stat4] = movebad(wnfilem,ssfilem,wnfignam40);
        end
        if strcmp(p.usel,'y') %
            [stat1,stat2,stat3,stat4] = movebad(wnfilel,ssfilel,wnfignam400);
        end
...
if strcmp(p.SaveTF,'yes')
    tfNum = str2num(p.tf.tfn);
    if strcmp(p.use,'y') %high fre
        tfcorrfile = fullfile(p.harp.OutFolder,'TFCorr',p.harp.OutTFCorr);
        freq = freq(2:nf); % remove 0 freq
        save(tfcorrfile,'freq','MTFCorra','MTFCorr','TFCorr','depth','tfNum',...
            'SlopeLR','OffSetLR','LRr2','SlopeTS','OffSetTS','fTS','SlopeTSu','OffSetTSu',...
            'col','coh','cutlowHz','cuthighHz');
        fprintf('High band TF correction applied %g to %g Hz (col %d, coh %d)\n', ...
            cutlowHz,cuthighHz,col,coh);
        % save TF_Wind in Folder in Output
        tfnewfile = fullfile(p.harp.OutFolder,'TF_Wind',...
            [p.tf.tfn,'_',p.harp.Proj,p.harp.Site,'_',p.harp.Depl,'_TFnew.tf']);
        save(tfnewfile,'TFnew','-ascii','-tabs'); % save TFnew
        tfnewfig = fullfile(p.harp.OutFolder,'TF_Wind',...
            [p.tf.tfn,'_',p.harp.Proj,p.harp.Site,'_',p.harp.Depl,'_TFnewfig']);
        savefig(TFFig,tfnewfig) % save as fig
        tfnewfigpdf = fullfile(p.harp.OutFolder,'TF_Wind',...
            [p.tf.tfn,'_',p.harp.Proj,p.harp.Site,'_',p.harp.Depl,'_TFnewfig.pdf']);
        saveas(TFFig,tfnewfigpdf) % save as pdf

        %save TF_Kait
        if strcmp(p.usek,'y')
            tfnewfilek = fullfile(p.harp.OutFolder,'TF_Kait',...
                [p.tf.tfn,'_',p.harp.Proj,p.harp.Site,'_',p.harp.Depl,'_TFnewK.tf']);
            save(tfnewfilek,'TFnewK','-ascii','-tabs'); % save TFnew
            tfnewfigk = fullfile(p.harp.OutFolder,'TF_Kait',...
                [p.tf.tfn,'_',p.harp.Proj,p.harp.Site,'_',p.harp.Depl,'_TFnewfigK']);
            savefig(KTFFig,tfnewfigk)
            tfnewfigpdfk = fullfile(p.harp.OutFolder,'TF_Kait',...
                [p.tf.tfn,'_',p.harp.Proj,p.harp.Site,'_',p.harp.Depl,'_TFnewfigK.pdf']);
            saveas(KTFFig,tfnewfigpdfk)
        end
    end
    if strcmp(p.usem,'y') %mid fre
        ...
        freqm = freqm(2:nfm);
        save(tfcorrfilem,'freqm','MTFCorrm','TFCorrm','depth','tfNum',...
            'SlopeLRm','OffSetLRm','LRr2m','SlopeTSm','OffSetTSm','fTSm','SlopeTSum','OffSetTSum',...
            'col','coh','cutlowHzm','cuthighHzm');
        fprintf('Mid band TF correction applied %g to %g Hz\n', ...
            cutlowHzm,cuthighHzm);
        % save TF_Wind in Folder in Output
        tfnewfilem = fullfile(p.harp.OutFolder,'TF_Wind',...
            [p.tf.tfn,'_',p.harp.Proj,p.harp.Site,'_',p.harp.Depl,'_TFnewm.tf']);
        save(tfnewfilem,'TFnewm','-ascii','-tabs');
        tfnewfigm = fullfile(p.harp.OutFolder,'TF_Wind',...
            [p.tf.tfn,'_',p.harp.Proj,p.harp.Site,'_',p.harp.Depl,'_TFnewfigm']);
        savefig(TFFigm,tfnewfigm)
        tfnewfigmpdf = fullfile(p.harp.OutFolder,'TF_Wind',...
            [p.tf.tfn,'_',p.harp.Proj,p.harp.Site,'_',p.harp.Depl,'_TFnewfigm.pdf']);
    ...
    if strcmp(p.usel,'y')
        ...
        freql = freql(2:nfl);
        save(tfcorrfilel,'freql','MTFCorrl10','TFCorrl10','depth','tfNum',...
            'SlopeLRl','OffSetLRl','LRr2l','SlopeTSl','OffSetTSl','fTSl','SlopeTSul','OffSetTSul',...
            'col','coh','cutlowHzl','cuthighHzl');
        fprintf('Low band TF correction applied %g to %g Hz\n', ...
            cutlowHzl,cuthighHzl);
        % save TF_Wind in Folder in Output
        tfnewfilel = fullfile(p.harp.OutFolder,'TF_Wind',...
            [p.tf.tfn,'_',p.harp.Proj,p.harp.Site,'_',p.harp.Depl,'_TFnewl.tf']);
        save(tfnewfilel,'TFnewl','-ascii','-tabs');
        tfnewfigl = fullfile(p.harp.OutFolder,'TF_Wind',...
            [p.tf.tfn,'_',p.harp.Proj,p.harp.Site,'_',p.harp.Depl,'_TFnewfigl']);
        savefig(TFFigl,tfnewfigl)
        tfnewfigpdfl = fullfile(p.harp.OutFolder,'TF_Wind',...
            [p.tf.tfn,'_',p.harp.Proj,p.harp.Site,'_',p.harp.Depl,'_TFnewfigl.pdf']);
...
%% close the run log
% Everything printed above is in this file. If the script errors before
% reaching here the diary is left open on purpose - the next run's
% startRunLog closes it, so the failed run's output is still on disk.
if exist('runLogFile','var') && ~isempty(runLogFile)
