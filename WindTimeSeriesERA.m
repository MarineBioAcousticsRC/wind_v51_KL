...
for yr = 2019 : 2019
    ...
    % site
    site = 'KOA';
    outdir = 'H:\Wind_Data\GofAK';  % Output file directory
    disp([site,' ',year]);
    ...
    slat25 = (round((slat+.25) * 4))/4 - .25;
    slon25 = (round((slon+.25) * 4))/4 - .25;
    ...
    for month = 1:12
        ...
        isfile = exist(f,'file');
        if isfile == 2
            %n = ncinfo(f); % read data for one month
            tihr = ncread(f,'time');
            uwnd = ncread(f,'u10');
            vwnd = ncread(f,'v10');
            for ihr = 1 : length(tihr)
                ivec = ivec + 1;
                dnvec(ivec) = era5time(tihr(ihr));
                uwvec(ivec) = uwnd(ilon,ilat,ihr);
                vwvec(ivec) = vwnd(ilon,ilat,ihr);
            end

        else
            disp(['Not Valid file:  ',year,m,1]);
        end
        disp(['Done with:  ',num2str(datevec(dnvec(ivec)))]);
    end
    % plot wind speed
    figure
    subplot(2,1,1)
    ...
    xlabel(' Month ');
    %save results
    fs = fullfile(outdir,year,[site,'windvecERA']);
