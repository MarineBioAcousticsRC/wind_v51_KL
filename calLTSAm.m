function [ptimem,mpwrm,mpwrtfm,freqm,eltsam] = calLTSAm()
...
LTSAFolderm = [PARAMS.ltsa.LTSAFolder,'mid'] ;
OutFolder = PARAMS.harp.OutFolder ;
OutName2 = PARAMS.harp.OutName2;
NA = PARAMS.harp.NA ;     % number of time slices (spectral averages) to read per raw file
tres = PARAMS.harp.tres ;
%
[fn_files, fn_pathname] = uigetfile(LTSAFolderm,'Pick Mid-LTSA(s)',...
    'MultiSelect','on');
%
% get TF
tf_freq = PARAMS.tf.freq;
tf_uppc = PARAMS.tf.uppc;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
