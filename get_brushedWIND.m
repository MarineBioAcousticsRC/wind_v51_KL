function [brushDate, bFlag] = get_brushedWIND(hFig)
...
selLine = arrayfun(@(x) hBrushAll(x).Marker == 'o', 1:length(hBrushAll), 'UniformOutput', false);
if iscell(selLine)
...
if ~isempty(brushID)
    ...
    bFlag = 1;
    brushDate = hBrush.ZData(brushID);
