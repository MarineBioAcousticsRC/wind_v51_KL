...
dlet = 'Z:'; % Letter for SNOWMAN drive
dlet = 'W:'; % Letter for SNOWMAN drive
dlet = 'X:'; % Letter for SNOWMAN drive
% dlet = 'L:\Shared drives\MBARC_All\Code\LTSA'
global PARAMS
...
% PARAMS.harp.Site = 'CCE1';
% PARAMS.harp.Site = '_CINMS_B';
% PARAMS.harp.Site = 'CINMS_C';
PARAMS.harp.Site = 'DT';
% PARAMS.harp.Site = 'CCE1_SB';
PARAMS.harp.Depl = '08';
PARAMS.harp.Short = PARAMS.harp.Depl;
% PARAMS.harp.band = 'high'; % high mid or low
PARAMS.harp.band = 'mid'; % high mid or low
PARAMS.harp.harpDataSummaryCSV = 'L:\Shared drives\Wind_deltaTF\HARPdataSummaryWIND.csv';
PARAMS.harp.harpDataSummaryCSV = 'H:\Wind_TF\HARPdataSummaryWIND.csv';
...
PARAMS.tf.TFsFolder = 'L:\Shared drives\MBARC_TF';
PARAMS.tf.TFsFolderOld = 'H:\Harp_TF';
% PARAMS.tf.TFsFolderOld = 'H:\Harp_TF\OLD\';
PARAMS.harp.OutFolder = 'H:\Wind_TF\Output\Kona_TF';
OutFolder = 'H:\Wind_TF\Output\Kona_TF';
...
if strcmp(fn_pathh(end-4:end),'high')
    fn_path = [fn_pathh(end-4:end),'mid'];
else
    fn_path = [fn_pathh,'mid'];
end
mstat = mkdir(fn_path);
[fn_filem, fn_pathm] = uigetfile([dlet,'\*.ltsa'],'Pick LTSA(s)',...
    'MultiSelect','on');
% sfm = size(fn_filem);
if ~iscell(fn_filem)
    ...
    k = strfind(fn_filem,'disk');
    fn_filenew = [fn_filem(1:k+3),'0',...
        num2str(ndisk),'*.ltsa'];
    %     if isempty(k)
    %         k = strfind(fn_filem,'_d01');
    %         fn_filenew = [fn_filem(1:k+3),'*.ltsa'];
    %     end
    %     fn_filenew = [fn_filem(1:end-20),'0',...
    %         num2str(ndisk),fn_filem(end-17:end)];
    id = 1; % count number of disks
    for i = 1 : 16 % max number of disks
        fn_files{1,id} = fn_filenew; id = id + 1;
        fnew = fullfile(fn_pathnew,fn_filenew);
        [~,msg] = copyfile(fnew,fn_path) ; % source destination
        disp([msg,' ',fnew]);
        ...
        if ndisk < 10
            ...
            fn_pathnew = [fn_pathnew(1:end-8),...
                num2str(ndisk),fn_pathnew(end-5:end)];
            fn_filenew = [fn_filem(1:k+3),...
                num2str(ndisk),'*.ltsa'];
            %             fn_filenew = [fn_filenew(1:end-20),...
            %                 num2str(ndisk),fn_filenew(end-17:end)];
        end
    end
else
    for j = 1:length(fn_filem)  
      fnew = fullfile(fn_pathm,fn_filem{1,j});
%       copyfile(fnew,fn_path) ; % sourc
    end
    fn_files = fn_filem;
end
% end
fn_pathname = fn_path;  % point to the 'mid' directory
% save mid file names
% midfile = fullfile(PARAMS.harp.OutFolder,'WindNoiseMat',PARAMS.harp.OutName1);
% save(midfile,'fn_files','fn_path') % save all workspace
sfn = size(fn_files);
if sfn(2) > 1
...
if ~exist('suggestedTFPath','var')
    suggestedTFPath = 'L:\Shared drives\MBARC_TF\';
end
[tf_file, tf_pathname ] = ...
...
save(fullfile(OutFolder,'WindNoiseMat',OutName),...
