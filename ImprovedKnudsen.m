...
for i = 1:3 % wind less than 5 m/s
    for iif = 1:4 % 100-400 Hz
        nl(i,iif) = 59.4 + (n *20* log10(ms(i))) + 10 * log10(f(iif)) - tl(iif);% m = 1
    end
     for iif = 5:9 % 500-1000 Hz
        nl(i,iif) = 54 + (n *20* log10(ms(i))) - m*10 * log10(f(iif)) - tl(iif);
     end
     for iif = 10:length(f) % > 1000 Hz
        nl(i,iif) = 54 + (n *20* log10(ms(i))) - 17 * log10(f(iif)) - tl(iif); %m = 1.7
    end
end
n = 1.25;
for i = 4:length(ms) % wind > than 5 m/s
    for iif = 1:4 % 100-400 Hz
        nl(i,iif) = 49.4 + (n *20* log10(ms(i))) + 10 * log10(f(iif)) - tl(iif); %m=1
    end
     for iif = 5:9 % 500-1000 Hz
        nl(i,iif) = 44 + (n *20* log10(ms(i))) - m*10 * log10(f(iif)) - tl(iif);
     end
     for iif = 10:length(f) % > 1000 Hz
        nl(i,iif) = 44 + (n *20* log10(ms(i))) - 17 * log10(f(iif)) - tl(iif); %m = 1.7
    end
end
% plot Knudsen curves
for i = 1: length(ms)
...
i=5;
semilogx(1000*f, nl(i,:),'r','LineWidth',2); % ss = 4 is log10(ms) = 1
% Thermal Noise curve
for iif = 1:length(f)
