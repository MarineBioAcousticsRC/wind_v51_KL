...
for iw = 1 % wind less than 1 m/s
    for iifm = 1:10 % freq 10-100 Hz in 10 Hz steps
        ofac = exp(-depth/bof)*aof*(12-iifm/10)/12; % depth and frequency
...
for iw = 2 % wind 1 - 2.5 m/s
    for iifm = 1:10 % freq 100-400 Hz
        ofac = exp(-depth/bof)*aof*(12- iifm/10)/12;
...
for iw = 1 % wind less than 1 m/s
    for iif = 1:4 % freq 100-400 Hz
        ...
        nl(iw,iif) = ofac + off+5.4 + (n*20* log10(ms(iw))) + 3 *m* log10(f(iif)) ; 
...
for iw = 2 % wind 1 - 2.5 m/s
    for iif = 1:4 % freq 100-400 Hz
        ofac = exp(-depth/bof)*aof*(12-iif)/12;
        nl(iw,iif) = ofac + off-0.14 + (n*20* log10(ms(iw))) + 2.3 *m* log10(f(iif)) ; 
...
for iw = 3 % wind 2.5 - 4.5 m/s
    for iif = 1:4 % freq 100-400 Hz
        ofac = exp(-depth/bof)*aof*(12-iif)/12;
        nl(iw,iif) = ofac + off-2.81 + (n*20* log10(ms(iw))) + 1.6 *m* log10(f(iif)) ; 
...
for iw = 4 % wind 4.5 - 6.7 m/s
    for iif = 1:4 % 100-400 Hz
        ofac = exp(-depth/bof)*aof*(12-iif)/12;
        nl(iw,iif) = ofac + off+0.18 + (n *22* log10(ms(iw))) + 0.9 *m* log10(f(iif)) ; %m=1
...
for iw = 5 % wind 6.7 - 9.4 m/s
    for iif = 1:4 % 100-400 Hz
        ofac = exp(-depth/bof)*aof*(12-iif)/12;
        nl(iw,iif) = ofac + off-1.9 + (n *23* log10(ms(iw))) + 0.2 *m* log10(f(iif)) ; %m=1
...
for iw = 6 % wind 9.4 - 12.3 m/s
    for iif = 1:4 % 100-400 Hz
        ofac = exp(-depth/bof)*aof*(12-iif)/12;
        nl(iw,iif) = ofac + off-2.10 + (n *23* log10(ms(iw))) - 0.5 *m* log10(f(iif)) ; %m=1
...
for iw = 7 % wind > than 12 m/s
    for iif = 1:4 % 100-400 Hz
        ofac = exp(-depth/bof)*aof*(12-iif)/12;
        nl(iw,iif) = ofac + off-3.02 + (n *23* log10(ms(iw))) - 1.2 *m* log10(f(iif)) ; %m=1
...
for iw = 8 % wind > than 12 m/s
    for iif = 1:4 % 100-400 Hz
        ofac = exp(-depth/bof)*aof*(12-iif)/12;
        nl(iw,iif) = ofac + off-3.54 + (n *23* log10(ms(iw))) - 1.9 *m* log10(f(iif)) ; %m=1
...
for iw = 9 % wind > than 12 m/s
    for iif = 1:4 % 100-400 Hz
        ofac = exp(-depth/bof)*aof*(12-iif)/12;
        nl(iw,iif) = ofac + off-4.06 + (n *23* log10(ms(iw))) - 2.6 *m* log10(f(iif)) ; %m=1
...
for iw = 10 % wind > than 12 m/s
    for iif = 1:4 % 100-400 Hz
        ofac = exp(-depth/bof)*aof*(12-iif)/12;
        nl(iw,iif) = ofac + off-4.58 + (n *23* log10(ms(iw))) - 3.3 *m* log10(f(iif)) ; %m=1
...
for iw = 11 % wind > than 12 m/s
    for iif = 1:4 % 100-400 Hz
        ofac = exp(-depth/bof)*aof*(12-iif)/12;
        nl(iw,iif) = ofac + off-5.2 + (n *23* log10(ms(iw)))  - 4 *m* log10(f(iif)) ; %m=1
...
alpha = 0.036 * fp.^(3/2);    % [dB/km] f => [kHz]
alphah = alpha * h/1000;
eah = 10.^(-alphah/10);
