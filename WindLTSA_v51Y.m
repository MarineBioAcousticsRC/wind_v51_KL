...
global p
drivelet = 'G'; Proj = 'GofCA'; Site = 'CB'; Depl = '10';
...
if strcmp(Psave.usel,'y')
    if strcmp(Psave.calavgl,'y') % calculate WindNoise_low.mat file ?
        [ptimel,mpwrl,mpwrtfl,freql,eltsal,dfreql,dnew,wsnew,fs0l] = calLTSAl49(fn_filesl, fn_pathnamel);  % reads LTSA files to get averages and gets Wind model
    else
...
if strcmp(Psave.usem,'y')
    if strcmp(Psave.calavgm,'y') % calculate WindNoise_mid.mat file ?
        [ptimem,mpwrm,mpwrtfm,freqm,eltsam,dfreqm,dnew,wsnew,fs0m] = calLTSAm49(fn_filesm, fn_pathnamem);  % reads LTSAm files to get averages and gets Wind model
    else
...
if strcmp(Psave.use,'y')
    if strcmp(Psave.calavg,'y') % calculate WindNoise.mat file ?
        [ptime,mpwr,mpwrtf,freq,eltsa,dfreq,dnew,wsnew,fs0] = calLTSA49(fn_files, fn_pathname);  % reads LTSA files to get averages and gets Wind model
